...
def match_simple():
    """Snaps second selected object to first using a temp constraint"""
    selection = cmds.ls(selection=True)
    if selection:
        cmds.delete(cmds.parentConstraint(selection[0], selection[1]))
    else:
        LOG.error('Nothing selected!')


def match_selection():
    """Matches second selected to first selected (like Maya constraints)"""
    selection = cmds.ls(selection=True)
    ...
    if selection:
        if len(selection) == 2:
            ...
            object = selection[1]
            match(target, object)
            LOG.info('Matched {} to {}'.format(object, target))
        else:
            LOG.info('Please select two objects to match, second selected matched to first')
    else:
        LOG.error('Nothing selected, select at least source and target objects')


def match_all_selection():
    """Matches selected object(s) to last"""
    selection = cmds.ls(selection=True)
    ...
    if selection:
        if len(selection) > 1:
            ...
            LOG.info('Please select two or more objects to match to last selected object')
    else:
        LOG.error('Nothing selected, select at least source and target objects')


def match(target, object):
    """Match object to target (like Maya constraints)
    ...
    if not cmds.objExists(object):
        raise Exception('Object {} does not exist!'.format(object))

    # Get target matrix
    targetMat = cmds.xform(target, q=True, ws=True, matrix=True)

    # Set source matrix
    cmds.xform(object, ws=True, matrix=targetMat)
