...
def createEylidJoints(vtxSelection, *args, **kwargs):
    ...
    for i in range(len(vtxSelection)):
        ...
        cmds.select(cl=1)
        vtxJnt = cmds.rename(cmds.joint(), baseName + args[0] + "_skn" + str(i))
        ...
        cmds.select(cl=1)
        centerJnt = cmds.rename(cmds.joint(), baseName + args[0] + "_jnt")
        cmds.xform(centerJnt, ws=True, t=eyeCenterPos)
...
def createAimedAtLocators(_constraintObjects, _targetPositions, _upVectorObject, *args, **kwargs):
    ...
    for i in range(len(_targetPositions)):
        ...
        targetLocator = cmds.rename(cmds.spaceLocator()[0], baseName + args[0] + "_loc")
...
upperEyelidParentJoints, upperEyelidSlideJoints = createEylidJoints(vtxSelectionUpperEyelid, "Upper")
...
aimLocators = createAimedAtLocators(upperEyelidParentJoints, upperEyelidSlideJoints, upVectorObject, "Upper")
print(aimLocators)